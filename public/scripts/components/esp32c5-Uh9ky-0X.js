...
class h extends i {
  ...
  async getChipDescription(E) {
    const t = await this.getPkgVersion(E);
    let _;
    t === 0 ? _ = "ESP32-C5" : _ = "unknown ESP32-C5";
    const S = await this.getMajorChipVersion(E), s = await this.getMinorChipVersion(E);
    return `${_} (revision v${S}.${s})`;
  }
  ...
  async getCrystalFreq(E) {
    ...
    let S;
    return _ > 45 ? S = 48 : _ > 33 ? S = 40 : S = 26, Math.abs(S - _) > 1 && E.info("WARNING: Unsupported crystal in use"), S;
  }
