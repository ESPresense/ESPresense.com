...
class A extends s {
  constructor() {
    ...
    ], this.UF2_FAMILY_ID = 2010665156, this.EFUSE_MAX_KEY = 5, this.KEY_PURPOSES = {
      0: "USER/EMPTY",
      1: "ECDSA_KEY",
      2: "XTS_AES_256_KEY_1",
      3: "XTS_AES_256_KEY_2",
  ...
  async getPkgVersion(_) {
    return await _.readReg(this.EFUSE_BLOCK1_ADDR + 8) >> 26 & 7;
  }
  async getMinorChipVersion(_) {
    return await _.readReg(this.EFUSE_BLOCK1_ADDR + 8) >> 0 & 15;
  }
  async getMajorChipVersion(_) {
    return await _.readReg(this.EFUSE_BLOCK1_ADDR + 8) >> 4 & 3;
  }
  async getChipDescription(_) {
    const S = await this.getPkgVersion(_);
    let t;
    S === 0 ? t = "ESP32-C61" : t = "unknown ESP32-C61";
    const E = await this.getMajorChipVersion(_), i = await this.getMinorChipVersion(_);
    return `${t} (revision v${E}.${i})`;
  }
  async getChipFeatures(_) {
    return ["WiFi 6", "BT 5"];
  }
  async readMac(_) {
    let S = await _.readReg(this.MAC_EFUSE_REG);
    S = S >>> 0;
    let t = await _.readReg(this.MAC_EFUSE_REG + 4);
    t = t >>> 0 & 65535;
    const E = new Uint8Array(6);
    return E[0] = t >> 8 & 255, E[1] = t & 255, E[2] = S >> 24 & 255, E[3] = S >> 16 & 255, E[4] = S >> 8 & 255, E[5] = S & 255, this._d2h(E[0]) + ":" + this._d2h(E[1]) + ":" + this._d2h(E[2]) + ":" + this._d2h(E[3]) + ":" + this._d2h(E[4]) + ":" + this._d2h(E[5]);
  }
}
export {
  A as ESP32C61ROM
