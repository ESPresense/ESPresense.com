...
class U extends r {
  constructor() {
    ...
    ], this.EFUSE_VDD_SPI_REG = this.EFUSE_BASE + 52, this.VDD_SPI_XPD = 16, this.VDD_SPI_TIEH = 32, this.VDD_SPI_FORCE = 64, this.UF2_FAMILY_ID = 3218951918, this.EFUSE_MAX_KEY = 5, this.KEY_PURPOSES = {
      0: "USER/EMPTY",
      1: "RESERVED",
      2: "XTS_AES_256_KEY_1",
      3: "XTS_AES_256_KEY_2",
    ...
    }, this.UART_CLKDIV_MASK = 1048575, this.UART_DATE_REG_ADDR = 1610612856, this.FLASH_WRITE_SIZE = 1024, this.BOOTLOADER_FLASH_OFFSET = 4096, this.FLASH_SIZES = {
      "1MB": 0,
      "2MB": 16,
      "4MB": 32,
      "8MB": 48,
  ...
  async getPkgVersion(_) {
    const s = this.EFUSE_BLOCK1_ADDR + 16;
    return await _.readReg(s) >> 0 & 15;
  }
  async getMinorChipVersion(_) {
    const s = await _.readReg(this.EFUSE_BLOCK1_ADDR + 12) >> 20 & 1, i = await _.readReg(this.EFUSE_BLOCK1_ADDR + 4 * 4) >> 4 & 7;
    return (s << 3) + i;
  }
  ...
  async getChipDescription(_) {
    const E = {
      ...
      102: "ESP32-S2FNR2",
      100: "ESP32-S2R2"
    }, s = await this.getFlashCap(_) + await this.getPsramCap(_) * 100, t = await this.getMajorChipVersion(_), i = await this.getMinorChipVersion(_);
    return `${E[s] || "unknown ESP32-S2"} (revision v${t}.${i})`;
  }
  async getFlashCap(_) {
    return await this.getFlashVersion(_);
  }
  async getPsramVersion(_) {
    const s = this.EFUSE_BLOCK1_ADDR + 12;
    return await _.readReg(s) >> 28 & 15;
  }
  async getPsramCap(_) {
    return await this.getPsramVersion(_);
  }
  async getBlock2Version(_) {
    const s = this.EFUSE_BLOCK2_ADDR + 16;
    return await _.readReg(s) >> 4 & 7;
  }
  async getChipFeatures(_) {
    ...
    }, t = await this.getFlashCap(_), i = s[t] || "Unknown Embedded Flash";
    E.push(i);
    const S = {
      0: "No Embedded Flash",
      1: "Embedded PSRAM 2MB",
      2: "Embedded PSRAM 4MB"
    }, h = await this.getPsramCap(_), R = S[h] || "Unknown Embedded PSRAM";
    E.push(R);
    const e = {
      0: "No calibration in BLK2 of efuse",
      1: "ADC and temperature sensor calibration in BLK2 of efuse V1",
      2: "ADC and temperature sensor calibration in BLK2 of efuse V2"
    }, n = await this.getBlock2Version(_), a = e[n] || "Unknown Calibration in BLK2";
    return E.push(a), E;
  }
  ...
  async readMac(_) {
    ...
    return t[0] = s >> 8 & 255, t[1] = s & 255, t[2] = E >> 24 & 255, t[3] = E >> 16 & 255, t[4] = E >> 8 & 255, t[5] = E & 255, this._d2h(t[0]) + ":" + this._d2h(t[1]) + ":" + this._d2h(t[2]) + ":" + this._d2h(t[3]) + ":" + this._d2h(t[4]) + ":" + this._d2h(t[5]);
  }
  getEraseSize(_, E) {
    return E;
  }
  async usingUsbOtg(_) {
    return (await _.readReg(this.UARTDEV_BUF_NO) & 255) === this.UARTDEV_BUF_NO_USB_OTG;
  }
  async postConnect(_) {
